...
Update: I haven't been using my thermal cameras overnight yet. It's been bothering me that I might fall asleep and leave them to be exposed and damaged by the sun. So, I've been looking for a solution that will automatically turn the camera lense away from the sun and into a shaded area before daylight, all for cheap. I've made some progress: I found a pan-tilt two servo set with some supports. I also have been looking into options for cheaply controlling the servos while having it on the network. Along the way, I've come across some curious issues, such as the controller boards mostly being for smaller servos and requiring less power as well as not coming with the required power supplies. After at least two hours searching for just the power supply, the closest wall adapter I could find that wasn't too expensive and wasn't the type that you just modified the voltage by hand (if you bumped one of those you'd ruin the driver board) had a max current of 3 amps...I'm just about to give up and go with a buck converter (not efficient). So now I'm just finishing up on my understanding on how I'll connect the driver board with an Arduino or RPi. The code for controlling the servos is pretty simple, will be using output from the Arduino or RPI GPIO headers (if I get an RPi Zero, I'll get one of the solderless headers to go with it) for the data line and will be sending the ground of the power supply through the ground on the Arduino or RPi to provide a common reference point...etc. Overall, I'll hopefully keep this entire hardware update under $150...hopefully. OK, I'm punting. Instead of designing a overly complex solution that would consist of an adapter and a buck step-down converter, I've emailed the servo board manufacturer for power supply suggestions since they don't have one packaged with the product. I'll wait.
